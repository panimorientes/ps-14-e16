...
public class ProfileInfo extends JPanel {
	...
	public ProfileInfo() {
		...
		actualData = DataProfileManager.getData();
		...
		for (int i = 0; i < textFieldVector.length; i++) {
			JPanel panel = new JPanel((LayoutManager) new FlowLayout(
					FlowLayout.LEFT));
			panel.add(new JLabel(DataProfileManager.dataLabels[i] + ": "));
	 ...
	 * 
	 * @param change
	 *            , if true, set the change, if false, dont set.
	 */
	public void setChanges(boolean change) {
		if (!change) {
			...
			fillInfo(newData);
			actualData = newData;
			DataProfileManager.updateData(actualData);
		}
	}
}
