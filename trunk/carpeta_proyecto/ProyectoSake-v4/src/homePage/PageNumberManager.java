...
public class PageNumberManager extends JPanel {
	...
	public PageNumberManager(SearchResultPage p) {
		...
		if (nPag == 1) {
			this.add(new JLabel((PropertiesManager
					.getProperties(PropertiesManager.PNM_LEFT_LABEL))));
			addLabel("1");
			this.add(new JLabel((PropertiesManager
					.getProperties(PropertiesManager.PNM_RIGHT_LABEL))));
		}

		else if (nPag < MAX_PAGES_NUMBER && nPag != 1) {

			addLabel((PropertiesManager
					.getProperties(PropertiesManager.PNM_LEFT_LABEL))
					+ (PropertiesManager
							.getProperties(PropertiesManager.PNM_PREVIUS_LABEL)));
			...
			addLabel((PropertiesManager
					.getProperties(PropertiesManager.PNM_NEXT_LABEL))
					+ (PropertiesManager
							.getProperties(PropertiesManager.PNM_RIGHT_LABEL)));
		}
	...
	private void addLabel(final String pagN) {
		...
		final int goToPag;
		int actPag = 0;
		if (!(pagN.equals((PropertiesManager
				.getProperties(PropertiesManager.PNM_LEFT_LABEL))
				+ (PropertiesManager
						.getProperties(PropertiesManager.PNM_PREVIUS_LABEL))) || pagN
				.equals((PropertiesManager
						.getProperties(PropertiesManager.PNM_NEXT_LABEL))
						+ (PropertiesManager
								.getProperties(PropertiesManager.PNM_RIGHT_LABEL))))) {
		...
		if (pagN.equals((PropertiesManager
			...
			goToPag = goTo;
		} else if (pagN.equals((PropertiesManager
				.getProperties(PropertiesManager.PNM_NEXT_LABEL))
				+ (PropertiesManager
						.getProperties(PropertiesManager.PNM_RIGHT_LABEL)))) {
			int goTo = mySearch.getPag() + 1;
